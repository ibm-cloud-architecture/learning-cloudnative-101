...
1. Verify the [toolchains and tool integrations that are available](https://cloud.ibm.com/docs/ContinuousDelivery?topic=ContinuousDelivery-cd_about) in your region and IBM Cloud environment. A toolchain is a set of tool integrations that support development, deployment, and operations tasks.

1. You need a Kubernetes cluster and an API key. You can create them by using either the UI or the CLI. You can create from the [IBM Cloud Catalog](https://cloud.ibm.com/kubernetes/catalog/create)

1. Create a container registry namespace to deploy the container we are goign to build. Youc an create from the [Container Registry UI](https://cloud.ibm.com/kubernetes/registry/main/namespaces)

1.  Create the API key by using the string that is provided for your key name.
    ```bash
...
4. Click **Add a Tool** and click **Git Repos and Issue Tracking**. 
    ...
    - In the **Source repository URL** field, type `https://us-south.git.cloud.ibm.com/bakribbs/hello-tekton.git`.
...
7. Click **Delivery Pipeline** to open the Tekton Delivery Pipeline dashboard. Click the **Definitions** tab and complete these tasks:
  ...
  - Click **Save**
1. Click the **Worker** tab and select the private worker that you want to use to run your Tekton pipeline on the associated cluster. Either select the private worker you set up in the previous steps, or select the **IBM Managed workers in DALLAS** option.
  ![Worker tab](./images/Tekton_Worker.png)
   - Click **Save**
8. Click the **Triggers** tab, click **Add trigger**, and click **Git Repository**. Associate the trigger with an event listener: 
...
- Click **Save**
9. On the **Triggers** tab, click **Add trigger** and click **Manual**. Associate that trigger with an event listener:
...
10. Click the **Environment properties** tab and define the environment properties for this tutorial. To add each property, click **Add property** and click **Text property**. Add these properties:
   - `apikey`: Type the API key that you created earlier in this tutorial.
   - `cluster`: Type the name of the {{>productName key="ibmCloudContainerSvc"}} cluster that you created.
   - `clusterNamespace`: Type the namespace in your cluster where the app will be deployed. The default is prod.
   - `clusterRegion`: Type the region where your {{>productName key="ibmCloudContainerSvc"}} cluster is located. The default is US-South.
   - `registryNamespace`: Type the {{>productName key="ibmCloudContainerRegistry"}} namespace where the app image will be built and stored. To use an existing namespace, use the CLI and run `ibmcloud cr namespace-list` to identify all your current namespaces.
   - `registryRegion`: Type the region where your {{>productName key="ibmCloudContainerRegistry"}} is located. The default is US-South. To find your registry region, use the CLI and run `ibmcloud cr region`.
   ...
   - `revision`: The branch, tag  or Git commitfor the Git repository. 
...
2. Delete the cluster or discard the namespace from it. It is easiest to delete the entire namespace (Please do not delete the `default` namespace) by using the IBM Cloud™ Kubernetes Service CLI from a command-line window. However, if you have other resources that you need to keep in the namespace, you need to delete the application resources individually instead of the entire namespace. To delete the entire namespace, enter this command:
    ```bash
...
4. Delete the container images. To delete the images in your container image registry, enter this command in a command-line window:
    ```bash
