...
- If you can't access the NodePort from your computer and only http/80 then edit the argocd-server and add the flag `--insecure`
    ...
    ```
    Then create an ingress to expose the argocd service, in the following example replace `x.x.x.x` with worker node external public IP.
    ```yaml
