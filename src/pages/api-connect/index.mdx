...
There are multiple internal scenarios where apis are beneficial, but we will only cover two of them.  The first one is Regulatory Requirements and Industry Standards.  Using APIs gives you the ability to build your business processes to conform with the business regulations for the industry.  Not only is it an internal benefit, but an external one as well for any clients or business partners.  
...
| Managing APIs | Covers multiple data patterns used to implement event-driven architectures. | See [Presentation](#resources) |
| API Analytics| Covers multiple data patterns used to implement event-driven architectures. | See [Presentation](#resources) |

## Activities

| Topics                            | Description         | Link        |
| --------------------------------| ------------------  |:----------- |
| Creating a Proxy Rest API | Create a REST API definition that proxies an existing service. | [Create a Proxy API](https://www.ibm.com/support/knowledgecenter/SSMNED_beta/com.ibm.apic.apionprem.doc/tutorial_apionprem_apiproxy.html) |
| Mapping JSON Content | Use API Connect to map message content from one format or schema to another. | [Mapping JSON Content](https://www.ibm.com/support/knowledgecenter/SSMNED_beta/com.ibm.apic.apionprem.doc/tutorial_apionprem_mapping.html) |
| Importing an API | Import an existing OpenAPI 2.0 definition. | [Importing APIs](https://www.ibm.com/support/knowledgecenter/SSMNED_beta/com.ibm.apic.apionprem.doc/tutorial_apionprem_import_api.html) |
| Creating a Client Application | Create a client application using API Manager. | [Application Creation](https://www.ibm.com/support/knowledgecenter/SSMNED_beta/com.ibm.apic.apionprem.doc/tutorial_apionprem_create_application.html)  |
