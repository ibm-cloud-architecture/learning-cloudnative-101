...
- This will put the `invoke` statement in the `operation-switch` to bring up the cases for the switch.  
...
- Once the `invokes` are created, Click on the operation-switch and 
...
- Now we can choose an `Operation` to invoke. 
